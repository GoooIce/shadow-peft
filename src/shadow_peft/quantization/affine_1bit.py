...
def quantize_1bit_affine(
    ...
    q = torch.round((groups - w_min.unsqueeze(-1)) / scales.unsqueeze(-1))

    q = q.reshape(out_features, in_features // 32, 32).to(torch.int64)
    ...
    return packed, scales.to(dtype), w_min.to(dtype)
...
class QuantizedLinear1Bit(nn.Module):
    ...
    @classmethod
    def from_linear(cls, linear: nn.Linear, group_size: int = 128) -> QuantizedLinear1Bit:
        module = cls(
            linear.in_features,
            linear.out_features,
        ...
        with torch.no_grad():
            packed, scales, biases = quantize_1bit_affine(
                linear.weight.detach(), group_size
            )
...
class QuantizedEmbedding1Bit(nn.Module):
    ...
    @classmethod
    def from_embedding(
        cls, embedding: nn.Embedding, group_size: int = 128
        ...
        with torch.no_grad():
            packed, scales, biases = quantize_1bit_affine(
                embedding.weight.detach(), group_size
            )
            module.weight = packed
            module.scales = scales
            module.biases = biases
...
def quantize_model_1bit(
    ...
    for name, module in list(model.named_modules()):
        ...
        if isinstance(module, nn.Linear):
            new_module: nn.Module = QuantizedLinear1Bit.from_linear(module, group_size)
        else:
            new_module = QuantizedEmbedding1Bit.from_embedding(module, group_size)
