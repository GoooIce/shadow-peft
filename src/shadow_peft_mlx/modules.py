...
class ShadowInjectionModel(nn.Module):
    ...
    Math mirrors the torch implementation (`shadow_peft.modules.ShadowInjectionModel`):
    ...
    def __init__(
        ...
        *,
        ...
        self.injection_downs = (
            mx.random.normal((num_layers, hidden_size, injection_hidden_size))
            * initialization_std
        )
        # Zero-init so a fresh adapter is an exact identity transform.
        self.injection_ups = mx.zeros((num_layers, injection_hidden_size, hidden_size))
    ...
    def __call__(
        ...
        shadow_hidden_states: mx.array,
        layer_idx: int,
    ) -> mx.array:
        delta = hidden_states - shadow_hidden_states
        delta_t = delta @ self.injection_downs[layer_idx]
        delta_t = self.dropout(delta_t)
        delta_t = delta_t @ self.injection_ups[layer_idx]
...
class ShadowUpdateModel(nn.Module):
    ...
    Mirrors the torch implementation (`shadow_peft.modules.ShadowUpdateModel`):
      h_in = LN(hidden)
      ht = T(h_in)
      g = sigmoid(G(h_in))
